        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden flex flex-col">
            <div className="px-6 pt-5 pb-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              {/* Table Tabs with live count badges */}
              <div className="flex gap-1.5 p-1 bg-slate-100 rounded-2xl shrink-0">
                <button
                  onClick={() => setActiveConsoleTab("inbound")}
                  className={`px-4 py-1.5 rounded-xl text-[10px] font-bold transition-all flex items-center gap-1.5 ${activeConsoleTab === "inbound" ? "bg-white text-slate-800 shadow-xs" : "text-slate-400 hover:text-slate-600"}`}
                >
                  Inbound
                  {!loading && (
                    <span className="w-4 h-4 rounded-full bg-amber-400 text-white text-[8px] font-black flex items-center justify-center">
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-xl text-[10px] font-bold transition-all flex items-center gap-1.5 ${activeConsoleTab === "outbound" ? "bg-white text-slate-800 shadow-xs" : "text-slate-400 hover:text-slate-600"}`}
                >
                  Outbound
                  {!loading && (
                    <span className="w-4 h-4 rounded-full bg-blue-400 text-white text-[8px] font-black flex items-center justify-center">
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-xl text-[10px] font-bold transition-all flex items-center gap-1.5 ${activeConsoleTab === "pending" ? "bg-white text-slate-800 shadow-xs" : "text-slate-400 hover:text-slate-600"}`}
                >
                  Approvals
                      ...
                      <tr key={v.id} className="hover:bg-slate-50/40 transition-colors">
                        <td className="px-6 py-3.5">
                          ...
                          {v.priority && v.priority !== "Normal" && (
                            <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-full ${v.priority === "Urgent" ? "bg-red-100 text-red-700" : v.priority === "High" ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{v.priority}</span>
                          )}
                        </td>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50 text-xs">
                    {loading ? (
                      ...
                      <tr key={v.id} className="hover:bg-slate-50/40 transition-colors">
                        <td className="px-6 py-3.5 font-bold text-slate-800">{v.customer?.name ?? v.customerName}</td>
                    ...
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan={5} className="text-center py-8">
                          <p className="text-slate-400 font-semibold">No visits awaiting approval</p>
                          <p className="text-[10px] text-slate-300 mt-1">Visits flagged as &ldquo;Requires Approval&rdquo; will appear here</p>
                        </td>
                      </tr>
                    ) : dashboardData.pendingApprovals.map((v: any) => (
                      <tr key={v.id} className="hover:bg-slate-50/40 transition-colors">
                        <td className="px-6 py-3.5">
                          <p className="font-bold text-slate-800">{v.customerName}</p>
                          <p className="text-[10px] text-slate-400 font-semibold">{v.customerCode}</p>
                        </td>
                        <td className="px-4 py-3.5">
                          <span className={`inline-flex px-2 py-0.5 rounded-md font-bold text-[9px] ${v.visitType === "Inbound" ? "bg-amber-100 text-amber-800" : "bg-blue-100 text-blue-800"}`}>
                            {v.visitType}
                          </span>
                        </td>
                        <td className="px-4 py-3.5 text-slate-500 font-semibold">{v.purpose}</td>
                        <td className="px-4 py-3.5 text-slate-500 font-medium">{v.submittedBy}</td>
                        <td className="px-6 py-3.5 text-right">
                          <span className={`inline-flex px-2 py-0.5 rounded-md font-bold text-[9px] ${
                            v.priority === "Urgent" ? "bg-red-100 text-red-700" :
                            v.priority === "High" ? "bg-amber-100 text-amber-700" :
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200/60 shadow-sm flex flex-col gap-6">
            ...
            <div className="flex flex-col items-center justify-center pt-4 pb-2 w-full">
              ...
              {/* 2-Column Grid Legend — 3 subscription states only */}
              <div className="grid grid-cols-3 gap-4 w-full mt-8 px-2">
                <div className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#A855F7]" />
                  <span className="text-[10px] font-bold text-slate-500 lowercase">active</span>
                </div>
                <div className="flex items-center gap-2">
